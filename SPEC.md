...
## 附录 A：坐标对拍（scene Y-up ↔ 点云 -Y up）

> ⚠️ **两组坐标系必须实测对齐**，未完成前禁止跨坐标系混算数值。这是"玩家/传送飞到正确位置"的命根子。
...
2. 取 scene JSON 中已知坐标的锚点（如沙发 `position=[-0.5,0.4,-3.5]`），确认它在点云场景中的实际位置。
3. 推导/验证两组坐标的映射（含 up 轴翻转、原点对齐、可能的轴交换）。
4. 产出结论：映射关系 + 一个已验证的转换函数（或映射表）。

**对拍完成前**：
- `room_id` 恒为 `null`
- B 不输出 `position` 型动作，只用 `tp_id` 或纯文本
- A 不做"点云位置 → 房间"的坐标级归因
